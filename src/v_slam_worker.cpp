...
void VSlamWorker::init(ros::NodeHandle& nh, const VSlamWorkerParams& params,
                       unsigned int worker_id)
{
  params_ = params;
  worker_id_ = worker_id;
  // Setup subscriber.
  scan_sub_ = nh.subscribe(params_.assembled_cloud_sub_topic,
                           kScanSubscriberMessageQueueSize,
...
void VSlamWorker::scanCallback(const sensor_msgs::PointCloud2& cloud_msg_in)
{
  ...
  if (scan_.size() > 0u)
  {
    ...
    if (local_map_stamped_.find(cloud_msg_in.header.stamp) ==
        local_map_stamped_.end())
    {
      local_map_stamped_.insert(LocalMapStamped::value_type(
          cloud_msg_in.header.stamp,
          LocalMap(laser_slam::Pose(),
...
void VSlamWorker::poseCallback(const geometry_msgs::PoseStamped& pose_msg_in)
{
  ...
  pose_.T_w = laser_slam::SE3(
      Eigen::Quaternion<double>(
          pose_msg_in.pose.orientation.w, pose_msg_in.pose.orientation.x,
  ...
  if (local_map_stamped_.find(pose_msg_in.header.stamp) ==
      local_map_stamped_.end())
  {
    local_map_stamped_.insert(LocalMapStamped::value_type(
        pose_msg_in.header.stamp,
        LocalMap(pose_, std::vector<laser_slam_ros::PointCloud>(1, laser_slam_ros::PointCloud()))));
  }
  else
  {
